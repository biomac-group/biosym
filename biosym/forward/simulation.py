...
class SimulationEnvironment:
    def __init__(self, model, dt=0.01, integrator="RK4", n_steps=1000, stopping_criteria=None, initial_state="neutral", **kwargs):
        ...
        if type(initial_state) == str:
            ...
            if 'model' not in initial_state['states'] or 'gc_model' not in initial_state['states'] or 'actuator_model' not in initial_state['states']:
                raise ValueError("Simulation: initial state must contain 'model', 'gc_model', and 'actuator_model' keys.")
            if 'model' not in initial_state['constants'] or 'gc_model' not in initial_state['constants'] or 'actuator_model' not in initial_state['constants']:
                raise ValueError("Simulation: initial state must contain 'model', 'gc_model', and 'actuator_model' keys.")
    ...
    def _create_simulation_ode(self):
        ...
        def simulation_ode(model_state_vector, t, others, n_extforce, runs):
            ...
            model_state_vector_ = jnp.concatenate((model_state_vector, jnp.zeros(len(model_state_vector)//2), actuator_signals, contact_signals[0].flatten(), contact_signals[1].flatten()))
            states_ = {
        ...
        self.simulation_ode = lambda states, t, others: simulation_ode(states, t, others, self.model.ext_forces, self.model.run)                                                              
        self.simulation_ode = jax.jit(self.simulation_ode)
        # Try to cache the jit-compilation - might not make a difference
        self.simulation_ode(jnp.zeros(2*self.model.coordinates['n']), 0.0, (jnp.zeros(self.model.forces['n']), self.model.default_inputs['constants'], self.model.default_inputs['states']['actuator_model'], self.model.default_inputs['states']['gc_model'], self.model.default_inputs['states']['model'][2*self.model.coordinates['n']:]))
    ...
    def step(self, controls=None):
        ...
        def _step_internal(controls, state, dt, simulation_ode, n_coordinates=0):
            ...
            new_state_ = odeint(simulation_ode, states_[:2*n_coordinates], t, (controls, constants_, actuator_states_, gc_states_, states_[2*n_coordinates:]))
            # The current state is only the last value of the integration
            return new_state_[-1]
        
        if not hasattr(self, "_step"):
            self._step = lambda controls, state, dt: _step_internal(controls, state, dt, self.simulation_ode, self.model.coordinates['n'])
            self._step = jax.jit(self._step)


        #self.state['states']['model'] = self._step(controls, self.state, self.dt, self.simulation_ode)
        self.state['states']['model'] = self.state['states']['model'].at[:2*self.model.coordinates['n']].set(
    ...
    def reset(self, seed=None, **kwargs):
        ...
        if type(mode) == str:
            ...
            assert 'states' in mode and 'constants' in mode, "Simulation: initial state must contain 'states' and 'constants' keys."
            assert 'model' in mode['states'] and 'gc_model' in mode['states'] and 'actuator_model' in mode['states'], "Simulation: initial state must contain 'model', 'gc_model', and 'actuator_model' keys."
            assert 'model' in mode['constants'] and 'gc_model' in mode['constants'] and 'actuator_model' in mode['constants'], "Simulation: initial state must contain 'model', 'gc_model', and 'actuator_model' keys."

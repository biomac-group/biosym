...
def confun(states_list, globals_dict, settings, info):
    ...
    return globals_dict.speed - jnp.mean(states_list.states.model[:settings.get('nnodes_dur')-1,info['speed_var_idx']])  # Ensure the sum of step sizes equals the total duration
...
def jacobian(states_list, globals_dict, settings, info):
    ...
    c = jnp.arange(info['nnz'], dtype=settings['int_dtype']) * states_list[0].states.size() + info['speed_var_idx'] - 1 # The -1 seems pretty wrong, but we keep it here

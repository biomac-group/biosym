...
def plot_stick_figure(model, states, dt=0.01, frame=None, **kwargs):
    ...
    if isinstance(states, dict):
        ...
        if 'model' not in states['states'] or 'gc_model' not in states['states'] or 'actuator_model' not in states['states']:
            raise ValueError("Invalid states dictionary. Must contain 'model', 'gc_model', and 'actuator_model' keys.")
        if 'model' not in states['constants'] or 'gc_model' not in states['constants'] or 'actuator_model' not in states['constants']:
            raise ValueError("Invalid constants dictionary. Must contain 'model', 'gc_model', and 'actuator_model' keys.")
        ...
        raise ValueError("Invalid states format. Must be a dictionary or a list of dictionaries.")
    ...
    for i, joint in enumerate(model.positions):
        if case_ == "2D":
            l, = ax.plot(joint_positions[i, non_zero_axes[0]], joint_positions[i, non_zero_axes[1]], c='k', marker='o')
        else:
            l, = ax.plot(joint_positions[i, 0], joint_positions[i, 1], joint_positions[i, 2], c='k', marker='o')
        joints.append(l)
    # Plot all connections
    for connection in connections:
        if case_ == "2D":
            l, = ax.plot(joint_positions[connection][:, non_zero_axes[0]], joint_positions[connection][:, non_zero_axes[1]], c='k')
        else:
            l, = ax.plot(joint_positions[connection][:, 0], joint_positions[connection][:, 1], joint_positions[connection][:, 2], c='k')
        segments.append(l)
    ...
    ax.set_ylabel(f"Y-axis [m]")
    if case_ == "3D":
        ax.set_zlabel(f"Z-axis [m]")
    ...
    if n_frames == 1:
        ...
        def update(frame):
            ...
            for i, joint in enumerate(model.positions):
                if case_ == "2D":
                    joints[i].set_data([[anim_joint_positions[frame][i, non_zero_axes[0]]], [anim_joint_positions[frame][i, non_zero_axes[1]]]])
                else:
                    joints[i].set_data([[anim_joint_positions[frame][i, 0]], [anim_joint_positions[frame][i, 1]]])
                    joints[i].set_3d_properties(anim_joint_positions[frame][i, 2])
            for i, connection in enumerate(connections):
                if case_ == "2D":
                    segments[i].set_data([[anim_joint_positions[frame][connection][:, non_zero_axes[0]]], [anim_joint_positions[frame][connection][:, non_zero_axes[1]]]])
                else:
            ...
            if hascontact:
                if isinstance(states, list):
                    model.gc_model.plot(False, model, mode='update', ax=ax, case = case_, non_zero_axes=non_zero_axes, frame=frame, plot_objects=plot_objects)
      
                return ax.collections
        ani = animation.FuncAnimation(fig, update, frames=np.arange(n_frames), interval=dt, blit=False)
        plt.show()

        ...
        if mode == "init":
            ...
            if isinstance(states, list):
                for i in range(len(states)):
                    pcp, pbody, body_idx = self.get_cp_moment_arms(states[i]['states'], states[i]['constants'], model, return_positions=True)
                    pos_cps.append(pcp)
                ...
                if len(states['states']['model'].shape) == 1:
                    pcp, pbody, body_idx = self.get_cp_moment_arms(states['states'], states['constants'], model, return_positions=True)
                    pos_cps.append(pcp)
                    pos_bodies.append(pbody)   
                    cp_forces.append(self.get_cp_forces(states['states'], states['constants'], model)) 
                else:
                    for i in range(len(states['states'])):
                        pcp, pbody, body_idx = self.get_cp_moment_arms(states['states'][i], states['constants'][i], model, return_positions=True)
                        pos_cps.append(pcp)
                        pos_bodies.append(pbody)
                        cp_forces.append(self.get_cp_forces(states['states'][i], states['constants'][i], model))
            ...
            for i in range(len(self.pos_cps[0])):
                if case == "2D":
                    l, = ax.plot(self.pos_cps[0][i, non_zero_axes[0]], self.pos_cps[0][i, non_zero_axes[1]], c='k', marker='o')
                else:
                    l, = ax.plot(self.pos_cps[0][i, 0], self.pos_cps[0][i, 1], self.pos_cps[0][i, 2], c='k', marker='o')
                self.plot_markers.append(l)
                if case == "2D":
                    l, = ax.plot([self.pos_bodies[0][i, non_zero_axes[0]], self.pos_cps[0][i, non_zero_axes[0]]], [self.pos_bodies[0][i, non_zero_axes[1]], self.pos_cps[0][i, non_zero_axes[1]]], c='k')
                else:
                    l, = ax.plot([self.pos_bodies[0][i, 0], self.pos_cps[0][i, 0]], [self.pos_bodies[0][i, 1], self.pos_cps[0][i, 1]], [self.pos_bodies[0][i, 2], self.pos_cps[0][i, 2]], c='k')
                ...
                for j in range(i, len(self.pos_cps[0])):
                    if body_idx[i] == body_idx[j]:
                        if case == "2D":
                            l, = ax.plot([self.pos_cps[0][i, non_zero_axes[0]], self.pos_cps[0][j, non_zero_axes[0]]], [self.pos_cps[0][i, non_zero_axes[1]], self.pos_cps[0][j, non_zero_axes[1]]], c='k')
                        else:
                            l, = ax.plot([self.pos_cps[0][i, 0], self.pos_cps[0][j, 0]], [self.pos_cps[0][i, 1], self.pos_cps[0][j, 1]], [self.pos_cps[0][i, 2], self.pos_cps[0][j, 2]], c='k')
                ...
                self.factor = factor
                if case == "2D":
                    l, = ax.plot([self.pos_cps[0][i, non_zero_axes[0]], self.pos_cps[0][i, non_zero_axes[0]] + factor * cp_forces[0][i, non_zero_axes[0]]], [self.pos_cps[0][i, non_zero_axes[1]], self.pos_cps[0][i, non_zero_axes[1]] + factor * cp_forces[0][i, non_zero_axes[1]]], c='darkgreen')
                else:
                    l, = ax.plot([self.pos_cps[0][i, 0], self.pos_cps[0][i, 0] + factor * cp_forces[0][i, 0]], [self.pos_cps[0][i, 1], self.pos_cps[0][i, 1] + factor * cp_forces[0][i, 1]], [self.pos_cps[0][i, 2], self.pos_cps[0][i, 2] + factor * cp_forces[0][i, 2]], c='darkgreen')
            ...
            for i, joint in enumerate(plot_markers):
                if case == "2D":
                    joint.set_data([[self.pos_cps[frame][i, non_zero_axes[0]]], [self.pos_cps[frame][i, non_zero_axes[1]]]])
                else:
                    joint.set_data([[self.pos_cps[frame][i, 0]], [self.pos_cps[frame][i, 1]]])
                    joint.set_3d_properties(self.pos_cps[frame][i, 2])
            for i, line in enumerate(body_lines):
                if case == "2D":
                    line.set_data([[self.pos_bodies[frame][i, non_zero_axes[0]]], [self.pos_cps[frame][i, non_zero_axes[0]]]], [[self.pos_bodies[frame][i, non_zero_axes[1]]], [self.pos_cps[frame][i, non_zero_axes[1]]]])
                else:
            ...
            for i in range(len(self.pos_cps[frame])):
                for j in range(i, len(self.pos_cps[frame])):
                    if self.body_idx[i] == self.body_idx[j]:
                        ...
                        id += 1
                        if case == "2D":
                            line.set_data([[self.pos_cps[frame][i, non_zero_axes[0]]], [self.pos_cps[frame][j, non_zero_axes[0]]]], [[self.pos_cps[frame][i, non_zero_axes[1]]], [self.pos_cps[frame][j, non_zero_axes[1]]]])
                        else:
            ...
            for i, line in enumerate(force_lines):
                if case == "2D":
                    line.set_data([[self.pos_cps[frame][i, non_zero_axes[0]]], [self.pos_cps[frame][i, non_zero_axes[0]] + self.factor * self.cp_forces[frame][i, non_zero_axes[0]]]], [[self.pos_cps[frame][i, non_zero_axes[1]]], [self.pos_cps[frame][i, non_zero_axes[1]] + self.factor * self.cp_forces[frame][i, non_zero_axes[1]]]])
                else:
                    pos_a = self.pos_cps[frame][i]

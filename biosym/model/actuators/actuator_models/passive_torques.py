...
class PassiveTorques(BaseActuator):
    ...
    def __init__(self, joints_dict):
        ...
        self.idx_actuated_joints = jnp.array([i for i, ji in enumerate(joints_dict) if ji.get("damping", 0.0) > 0.0 or ji.get("stiffness", 0.0) > 0.0])# or ji.get("armature", 0.0) > 0.0]
    ...
    def get_actuated_joints(self):
        ...
        """
        return [ji['name'] for ji in self.joints_dict if ji.get("damping", 0.0) > 0.0 or ji.get("stiffness", 0.0) > 0.0]# or ji.get("armature", 0.0) > 0.0]
